...
import { BANNER_HISTORY_REQUEST } from "../../../reducers/banner";

const MainBannerHistory = ({}) => {
  const { st_loadMyInfoDone, me } = useSelector((state) => state.user);
  const { bannerHistory } = useSelector((state) => state.banner);
  ...
  return (
    <AdminLayout>
      ...
      <Wrapper margin={`10px 0px 0px 0px`}>
        <GuideUl>
          <GuideLi>메인배너 제어이력을 확인할 수 있습니다.</GuideLi>
          <GuideLi isImpo={true}>
      ...
      <Wrapper padding={`0px 50px`}>
        <Table
          ...
          rowKey="id"
          columns={col}
          dataSource={bannerHistory ? bannerHistory : []}
          size="small"
        />
      </Wrapper>
    </AdminLayout>
