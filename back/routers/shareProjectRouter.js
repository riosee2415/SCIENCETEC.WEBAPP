...
router.post("/create", async (req, res, next) => {
  ...
  try {
    ...
    console.error(error);
    return res.status(401).send("회원 조합 소개를 생성할 수 없습니다.");
  }
});
...
router.post("/update", async (req, res, next) => {
  ...
  try {
    ...
    console.error(error);
    return res.status(401).send("회원 조합 소개를 수정할 수 없습니다.");
  }
});
...
router.post("/image/update", isAdminCheck, async (req, res, next) => {
  ...
  try {
    ...
    console.error(error);
    return res.status(401).send("조합 이미지를 수정할 수 없습니다.");
  }
});
...
router.post("/delete", async (req, res, next) => {
  ...
  try {
    ...
    console.error(error);
    return res.status(401).send("회원 조합 소개를 삭제할 수 없습니다.");
  }
});
...
router.post("/under/image/update", isAdminCheck, async (req, res, next) => {
  const { id, imagePath } = req.body;
  ...
  try {
    ...
    console.error(error);
    return res.status(401).send("산하 조합 이미지를 수정할 수 없습니다.");
  }
});
